...
class ModelInputs:
    ...
    def __init__(self, transportation_matrix, locations, depot, vehicles):
        ...
        self.wgt=None #location weight
        self.vol=None #loction volume
        self.c = None #costs
        ...
        self.crate_costs()
        self.crate_location_volume()
        self.crate_location_weight()
        ...
        for row in self.vehicles.itertuples():
            self.Q[row.VEHICLE_NAME] = (
                VehicleInputs(
                    ...
                    VOLUME=row.VOLUME,
                    WEIGHT=row.WEIGHT,
                    MAX_TRAVEL_DIST=row.MAX_TRAVEL_DIST
                )
            )
    ...
    def crate_stop_times(self):
        ...
        '''
        self.s = {}
        StopInputs = collections.namedtuple('StopInputs', ['STOP_TIME_WH_HELPER'])
        for location in self.V:
            if location == self.depot_leave:
                ...
                stop_mins1 = stop['STOP_TIME_WH_HELPER'].iloc[0]

            self.s[location] = (
                StopInputs(
                    STOP_TIME_WH_HELPER=stop_mins1
                )
            )
    ...
    def crate_location_volume(self):
        '''
        Location volume
        :return:
        '''
        ...
        for location in self.N:
            stop = self.locations[self.locations['LOCATION_NAME'] == location]
            self.vol[location] = stop['VOLUME'].iloc[0]

    def crate_location_weight(self):
        '''
        Location weight
        :return:
        '''
        ...
        for location in self.N:
            stop = self.locations[self.locations['LOCATION_NAME'] == location]
            self.wgt[location] = stop['WEIGHT'].iloc[0]
    ...
    def crate_costs(self):
        ...
        '''
        self.c = {}
        CostInputs = collections.namedtuple('CostInputs', ['TRANS_COST_PER_MINUTE','FIXED_COST'])
        for row in self.vehicles.itertuples():
    ...
    def create_depot_time_windows(self):
        ...
        for location in self.N:
            ...
            a_t[location] = self.a[location] + self.s[location].STOP_TIME_WH_HELPER + self.t[location, self.depot_enter]
    ...
    def create_time_bound(self):
        ...
        for i, j in self.t.keys():
            self.M[i, j] = max(0, self.b[i] + self.s[i].STOP_TIME_WH_HELPER + self.t[i, j] - self.a[j])
