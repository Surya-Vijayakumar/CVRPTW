...
class Formulation:
    ...
    def __init__(self, K, V, N, t, q, s, locations, depot,
                 ...
                 a, b, M, Q, c, solver_time_limit_mins, mip_gap,vol,wgt):
    ...
    def create_model_formulation(self):
        ...
        self._create_vehicle_volume_constraint()
        self._create_vehicle_weight_constraint()
        self._create_maximum_travel_distance_constraint()
    ...
    def _create_vehicle_depot_leave_constraint(self):
        '''
        Each vehicle is visiting one location after leaving depot or staying back
        :return:
        '''
        for k in self.K:
            self.solver.Add(
        ...
        for k in self.K:
            self.solver.Add(
                self.solver.Sum([self.x[self.depot_leave, j, k] for j in self.N]) >= 0
            )
    ...
    def _create_vehicle_depot_arriving_constraint(self):
        ...
        for k in self.K:
            self.solver.Add(
                self.solver.Sum([self.x[i, self.depot_enter, k] for i in self.N]) >= 0
            )
    ...
    def _create_vehicle_capacity_constraint(self):
        '''
        Total orders in the vehicle can not exceed the total capacity
        :return:
        '''
        for k in self.K:
            self.solver.Add(
    ...
    def _create_vehicle_volume_constraint(self):
        '''
        ...
        for k in self.K:
            self.solver.Add(
                self.solver.Sum([self.vol[i]*self.x[i, j, k] for i in self.N for j in self.outgoing_arcs[i]]) <=
                self.Q[k].VOLUME
            )

    def _create_vehicle_weight_constraint(self):
        '''
        Total weight in the vehicle can not exceed the total capacity
        :return:
        '''
        for k in self.K:
            self.solver.Add(
                self.solver.Sum([self.wgt[i]*self.x[i, j, k] for i in self.N for j in self.outgoing_arcs[i]]) <=
                self.Q[k].WEIGHT
            )
    ...
    def _create_maximum_travel_time_constraint(self):
        ...
        for k in self.K:
            self.solver.Add(
                self.w[self.depot_enter, k] - self.w[self.depot_leave, k] <= self.Q[k].MAXIMUM_SOLO_TRAVEL_HOURS*60
            )
                #self.solver.Add(
                #    self.w[self.depot_enter, k] - self.w[i, k] + self.t[self.depot_leave, i] <= self.v[k]
                #)

    def _create_maximum_travel_distance_constraint(self):
        '''
        Total travel time can not exceed total travel time
        :return:
        '''
        for k in self.K:
            self.solver.Add(
                ((self.w[self.depot_enter, k] - self.w[self.depot_leave, k])/60)*15.5 <= self.Q[k].MAX_TRAVEL_DIST
            )
    ...
    def compile_results(self):
        ...
        for i, j in self.t.keys():
            for k in self.K:
                visit_orders.append({'PREVIOUS_LOCATION_NAME': i,
                                     ...
                                     'TOTAL_DISTANCE': (self.t[i, j]/60)*15.5})
        ...
        for idx, i in enumerate(self.V):
            for k in self.K:
                ...
                service_time = self.s[i].STOP_TIME_WH_HELPER
        ...
        solution = solution.merge(self.locations[['LOCATION_NAME', 'LATITUDE', 'LONGITUDE','WEIGHT','VOLUME']], how='left',
                                  on=['LOCATION_NAME'])
